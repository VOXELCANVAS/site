...
<html lang="en">
  ...
  <body onload = "checkmobile()">
        ...
        <ul class="nav nav-pills" style="font-size:20px;">
            ...
            <iframe id="container" frameborder=0 src="<?php
            $url = './build/';
            if($_GET['m'] || $_GET['location'] || $_GET['zoom'] || $_GET['image']){
              $url = $url . '?';
            }
            if($_GET['m']){
              $url = $url . 'm=' . $_GET['m'] . '&';
            ...
            if($_GET['location']){
              $url = $url . 'location=' . $_GET['location'] . '&';
            }
            if($_GET['zoom']){
              $url = $url . 'zoom=' . $_GET['zoom'] . '&';
            }
            if($_GET['image']){
              $url = $url . 'image=' . $_GET['image'] . '&';
            }
             echo $url;
             ?>"></iframe>
  </body>
